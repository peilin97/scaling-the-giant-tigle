...
public class Store implements Runnable {
   ...
   * @param successfulCount         count the number of successful requests
   * @param failedCount             count the number of failed requests
   * @param records                 track each request's record
   */
  ...
  private void sendPurchaseRequests() {
    while (purchaseCount < upperBoundPurchaseCount) {
      ...
      records.add(new RequestRecord(requestStartTime, latency, "POST", statusCode));
      purchaseCount++;
